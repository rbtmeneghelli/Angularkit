...
import { SharedMessages } from "../../app_entities/shared/shared-messages";
...
export const makeReplaceSpecialCharactersInRequest = (prop: string, spCharacter: string): string => {
...
export const somenteNumerosValidos = (value: string): string => {
    value = value.toString().replace(SharedVariables.REGEX_ONLY_NUMBER, '');
    return !!value ? (value !== '0' ? parseInt(value).toString() : '') : '';
}

export const getOnlyNumbers = (value: any): string => {
    return (value ?? '').toString().replace(SharedVariables.REGEX_ONLY_NUMBER, '');
}

export const getOnlyAlphaNumeric = (value: any): string => {
    return (value ?? '').toString().replace(SharedVariables.REGEX_ONLY_ALPHANUMERIC, '');
}

export const truncate = (source: string, size: number, concat: string): string => {
    return source.length > size ? source.slice(0, size - 1) + concat : source;
}

export const toBrazilFormat = (value: any, minimumFractionDigits: number = 8, maximumFractionDigits: number = 8): string => {
...
export const getNoRepeatedSpaceAlphanumeric = (value: any): string => {
    return (value ?? '').toString()
        .replace(SharedVariables.REGEX_NOT_REPEATED_SPACE, ' ')
        .replace(SharedVariables.REGEX_NOT_SPECIAL_CHARACTER, '');
}
...
export const formataNumeroToMilhar = (stNumero: string, blTrocarVirgulaPorPonto): string => {
    if (!!stNumero) {
...
export const convertStringToDateOrTimeOnly = (pDate?: string, isDate?: boolean): string => {
...
export const replaceAll = (word?: string, oldletter?: string, newLetter?: string): string => {
    if (!!word) {
...
export const convertPtBrDateStringToEnUsDateString = (dateValue: string): string => {
